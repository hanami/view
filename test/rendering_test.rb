...
describe Hanami::View do
  ...
  describe 'rendering' do
    it 'renders a template' do
      Test::HelloWorldView.render(format: :html).must_include %(<h1>Hello, World!</h1>)
    end

    it 'renders a template with context binding' do
      Test::RenderView.render(format: :html, planet: 'Mars').must_include %(<h1>Hello, Mars!</h1>)
    end

    # See https://github.com/hanami/view/issues/76
    it 'renders a template with different encoding' do
      Test::EncodingView.render(format: :html).must_include %(Configuração)
    end
    ...
    it 'renders a template according to the declared format' do
      Test::JsonRenderView.render(format: :json, planet: 'Moon').must_include %("greet":"Hello, Moon!")
    end

    it 'renders a template according to the requested format' do
      articles = [OpenStruct.new(title: 'Man on the Moon!')]

      rendered = Kiosk::Views::Articles::Index.render(format: :json, articles: articles)
      rendered.must_match %("title":"Man on the Moon!")

      rendered = Kiosk::Views::Articles::Index.render(format: :html, articles: articles)
    ...
    describe 'calling an action method from the template' do
      it 'can call with multiple arguments' do
        Test::RenderViewMethodWithArgs.render(format: :html).must_include %(<h1>Hello, earth!</h1>)
      end

      it 'will override Kernel methods' do
        Test::RenderViewMethodOverride.render(format: :html).must_include %(<h1>Hello, foo!</h1>)
      end

      it 'can call with block' do
        Test::RenderViewMethodWithBlock.render(format: :html).must_include %(<ul><li>thing 1</li><li>thing 2</li><li>thing 3</li></ul>)
      end
    end
    ...
    it 'binds given locals to the rendering context' do
      ...
      rendered = Kiosk::Views::Articles::Show.render(format: :html, article: article)
    ...
    #   rendered = Test::NestedView.render(format: :html)
    ...
    #   -> {
    #     Test::RenderViewWithMissingPartialTemplate.render(format: :html)
    #   }.must_raise(Hanami::View::MissingTemplateError)
