...
module Hanami
  class View
    ...
    class RenderEnvironment
      ...
      include Dry::Equalizer(:renderer, :inflector, :context)

      # attr_reader :renderer, :inflector, :context, :scope_builder, :part_builder
      attr_reader :renderer, :inflector, :context

      # def initialize(renderer:, inflector:, context:, scope_builder:, part_builder:)
      def initialize(renderer:, inflector:, context:)
        @renderer = renderer
        @inflector = inflector
        @context = context # do I still need for_render_env here?
